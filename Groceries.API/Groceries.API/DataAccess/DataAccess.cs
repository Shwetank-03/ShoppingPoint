    ...
    public class DataAccess : DbContext, IDataAccess
    {
        ...
        public List<Review> GetProductReviews(int productId)
        {
            ...
            catch (Exception ex)
            {
                ...
                // You can log the exception or perform any other necessary error handling here
                throw; // Rethrow the exception to trigger the error 500 response
            }
            return reviews;
        }
        ...
        public bool InsertProduct(Product product)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                ...
                string query = "INSERT INTO Products (Title, Price, Quantity, ImageName, CategoryId, OfferId) " +
                    "VALUES (@title, @price, @quantity, @imageName, @categoryId, @offerId);";
                command.CommandText = query;
        ...
        public object RemoveCartItem(int cartItemId)
        {
            using (SqlConnection connection = new SqlConnection(dbconnection))
            {
                SqlCommand command = new SqlCommand()
