...
export class UtilityService {
  ...
  applyDiscount(price: number, discount: number): number {
    let finalPrice: number = price - price * (20 / 100);
    return finalPrice;
  }
  ...
  calculatePayment(cart: Cart, payment: Payment) {
    ...
    if (payment.amountPaid > 500) payment.shippingCharges = 20;
    else if (payment.amountPaid > 2000) payment.shippingCharges = 100;
    else if (payment.amountPaid > 500) payment.shippingCharges = 50;
    else payment.shippingCharges = 20;
  }
